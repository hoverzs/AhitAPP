...
async function generateWithRetry(date: string) {
  try {
    ...
    cronError("Első generálási kísérlet hibája — nyers kivétel", {
      date,
      error: describeErrorForLog(firstError),
    });
    ...
    try {
      ...
      cronError("Második generálási kísérlet hibája — nyers kivétel", {
        date,
        error: describeErrorForLog(secondError),
      });
      throw secondError;
    }
  }
...
export async function runDailyCronGeneration(options?: {
  ...
  try {
    ...
    cronError("Végső cron catch — nyers kivétel részletek", {
      date,
